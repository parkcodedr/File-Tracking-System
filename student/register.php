...
if (isset($_POST['register'])) {
    ...
    if(empty($name) or empty($email) or empty($password) or empty($matno)) {
        $msg.="<div class='text-danger'> All fields are compulsory.</div>";
    }else if (!filter_var($email,FILTER_VALIDATE_EMAIL)) {
        $msg.="<div class='text-danger'> Invalid Email.</div>";
    
    }else if($userExist = $user->findOne(['email'=>$email])){
        $msg.="<div class='text-danger'> Email Already Registered .</div>";
        
    }else if($userExist = $user->findOne(['_id'=>$matno])){
        $msg.="<div class='text-danger'> Matric Number Already Registered .</div>";
        
    }else{

            $password=md5($password);
            
            $result = $user->insertOne(['_id'=>$matno,'name'=>$name,'email'=>$email,'password'=>$password]);
            if($result){
                    ...
                    <ul class="nav navbar-nav main-navbar-nav">
                        ...
                        <li class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li><a href="login.php" title="">STUDENT</a></li>
        ...
        <div class="container">
            <div class="row">
                <section class="category-content col-sm-9">
                    <h2 class="category-title col-md-offset-2">Register</h2>
                    <ul class="media-list">
                        <li class="media">
                            ...
                            <form class="form-horizontal" method="POST">
                                <div class="form-group">
                                  <label class="control-label col-sm-2" for="matno">Matno:</label>
                                  <div class="col-sm-10">
